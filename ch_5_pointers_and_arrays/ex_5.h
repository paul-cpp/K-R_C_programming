...
void ch5_getfloat(double* pd) {
  ...
  for (*pd = 0; isdigit(c) || c == 46;
       c = getch())  // 46 - Это точка в ascii таблице
  {
    if (c != 46) {
      pos++;
      *pd = 10 * *pd + (c - '0');
...
/**
 * ex 5.9
 * @brief day_of_year -  определяет день года по месяцу и дню (через указатели)
 */
...
/**
 * ex 5.9
 * @brief month_day - определяет месяц и день по дню года (через указатели)
 */
